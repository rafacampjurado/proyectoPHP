...
EX;
        ...
        if (5 - $puntuacion > 0) {
            for ($index = 0; $index < 5 - $puntuacion; $index++) {
                $res .= '<i class="fas fa-star empty"></i>';
            }
        }
        $res .= <<<EX
        </div>
...
function pintarPaginas($idProducto) {
    ...
    if ($numPaginas > 0) {
        ...
        for ($index = 0; $numPaginas > $index; $index++) {
            if ($index == 0) {
                $resultado .= '<li class="pag"><button id="' . ($index) . '" class="numpag" data-pagina="' . ($index + 1) . '">' . ($index + 1) . '</button></li>';
            } else {
                $resultado .= '<li class="pag"><button id="' . ($index + 2) . '" class="numpag" data-pagina="' . ($index + 1) . '">' . ($index + 1) . '</button></li>';
            }
        }
...
function pintarFacturas() {
    ...
    function tst() {
        $lnk = new PDO("mysql:host=localhost;dbname=foodnation", "root", "");
        $res = $lnk->prepare("SELECT * FROM facturas WHERE idUsuario = :id ;");
        $res->bindParam(":id", $_SESSION['idUsuario'], PDO::PARAM_INT);

        if ($res->execute()):
            if ($res->rowCount()):
                echo "<pre>" . print_r($res->fetchObject(), true) . "</pre>";
            else:
